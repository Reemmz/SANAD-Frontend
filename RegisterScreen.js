...
import {
  ...
  , SafeAreaView, Image, TouchableHighlight, Alert
} from 'react-native';
...
export default class RegisterScreen extends Component {
  ...
  async onSignUpPressed() {
    const { name, username, license, email, password } = this.state;
    if (name == "" || username == "" || license == "" || email == "" || password == "") {
    ...
    axios({
      ...
      .then(response => {
        ...
        if (data.success == true) {
          // alert(data.token_data.access_token);
          this.storeData(JSON.stringify(data.token_data.access_token), JSON.stringify(data.user.id));
  ...
  async storeData(token, user_id) {
    try {
      ...
      await AsyncStorage.multiSet([['ACCESS_TOKEN', token], ['USER_ID', user_id]], null);
